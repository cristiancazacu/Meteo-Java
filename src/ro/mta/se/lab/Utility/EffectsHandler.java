...
public class EffectsHandler {
    ...
    public static void PlayPathTransition(Node UI, double duration, double deplasament, int cycleCount)
    {

        UI.hoverProperty().addListener((ChangeListener<Boolean>) (observable, oldValue, newValue) ->{
            TranslateTransition transition = new TranslateTransition();
            double position = UI.getTranslateX();

            if (newValue)
            {
                transition.setDuration(Duration.seconds(duration));
                transition.setFromX(position);
                transition.setToX(deplasament);
                transition.setCycleCount(cycleCount);
                transition.setNode(UI);
                transition.play();
            }
            else {
                transition.setDuration(Duration.seconds(duration));
                transition.setFromX(deplasament);
